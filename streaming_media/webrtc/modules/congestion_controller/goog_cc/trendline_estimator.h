...
public:
  class INetworkStateChange
  {
  public:
    virtual INetworkStateChange();
    virtual void OnBwNormal(BandwidthUsage prev_state) = 0;
    virtual void OnBwUnderusing(BandwidthUsage prev_state) = 0;
    virtual void OnBwOverusing(BandwidthUsage prev_state) = 0;
  };
   ...
   * @param ob
   */
  void AddStateObserver(INetworkStateChange* ob);
  void OnState(BandwidthUsage hypothesis, BandwidthUsage prev_trend);
...
private:
  ...
  // @note xiaohu
  std::unordered_set<INetworkStateChange*> state_observers_;
};
}  // namespace webrtc
