...
void TrendlineEstimator::AddStateObserver(TrendlineEstimator::INetworkStateChange *ob)
{
  state_observers_.insert(ob);
}

void TrendlineEstimator::OnState(BandwidthUsage hypothesis,
                                 BandwidthUsage prev_trend)
{
  switch (hypothesis) {
    case BandwidthUsage::kBwNormal:
      for(auto ob : state_observers_)
        ob->OnBwNormal(prev_trend);
      break;
    case BandwidthUsage::kBwOverusing:
      for(auto ob : state_observers_)
        ob->OnBwOverusing(prev_trend);
      break;
    case BandwidthUsage::kBwUnderusing:
      for(auto ob : state_observers_)
        ob->OnBwUnderusing(prev_trend);
      break;
    default:
      break;
  }
...
void TrendlineEstimator::Detect(double trend, double ts_delta, int64_t now_ms) {
  ...
  BandwidthUsage lastState = hypothesis_;
